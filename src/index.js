...
logger.info(
  `Running as a proxy for SPDZ engine ${spdzHostName}:${spdzPortNum} with player id ${playerId}.`
)
...
if (serverType === 'websocket') {
  const io = new Io(webServer, { path: '/spdz/socket.io' }) //connection
  setupSpdzInteraction(io, '/spdzapi', spdzEngine, playerId)
